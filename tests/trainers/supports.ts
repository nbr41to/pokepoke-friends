...
const extractSupportCards = (html: string): SupportCard[] => {
  ...
  cardElements.forEach((element) => {
    ...
    if (!typeElement || typeElement.textContent !== 'サポート') return;
...
const processHtmlFiles = async () => {
  ...
  const outputFile = path.join(outputDir, 'supports.json');
  const allCards: SupportCard[] = [];
  ...
  for (const file of files) {
    ...
    const cards = extractSupportCards(html);
    console.log(`${file}: ${cards.length}枚のサポートカードを抽出しました`);
    allCards.push(...cards);
  }
  ...
  console.log(`合計: ${uniqueCards.length}枚のサポートカードを ${outputFile} に保存しました`);
};

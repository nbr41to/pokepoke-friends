...
const extractItemCards = (html: string): ItemCard[] => {
  ...
  cardElements.forEach((element) => {
    ...
    if (!typeElement || typeElement.textContent !== 'グッズ') return;
...
const processHtmlFiles = async () => {
  ...
  for (const file of files) {
    ...
    const cards = extractItemCards(html);
    console.log(`${file}: ${cards.length}枚のグッズカードを抽出しました`);
    allCards.push(...cards);
  }
  ...
  console.log(`合計: ${uniqueCards.length}枚のグッズカードを ${outputFile} に保存しました`);
};
